...
(function () {
  ...
  var goal = goals[key];
  var task = goal.tasks.filter(function (t) { return t.state === 'you' && t.options; })[0];
  ...
  var b = el('b', null, w[0] + (w[0] === 1 ? ' decision' : ' decisions'));
  greet.append('Morning, Swathi. ', b, ' across ' + w[1] + (w[1] === 1 ? ' goal' : ' goals') + ' need you.');
  ...
  var working = goal.tasks.filter(function (t) { return t.state === 'motion'; }).slice(0, 2);
  if (working.length) {
